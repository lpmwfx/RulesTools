...
from rust.checks.string_states import check as check_string_states
from rust.checks.magic_numbers import check as check_magic_numbers
from rust.checks.hardcoded_paths import check as check_hardcoded_paths
...
def scan_file(path: Path) -> list[Issue]:
    ...
    issues.extend(check_string_states(path, lines))
    issues.extend(check_magic_numbers(path, lines))
